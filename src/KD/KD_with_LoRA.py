...
class DistillationTrainer(Seq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        inputs["input_features"] = inputs["input_features"].to(dtype=torch.bfloat16, device=model.device).requires_grad_(True)
        if "labels" in inputs:
            inputs["labels"] = inputs["labels"].to(device=model.device)
        ...
        if self.model.training:
            with torch.no_grad():
                ...
                # Ensure the combined loss requires gradients
                loss = ((self.alpha * distill_loss) + ((1 - self.alpha) * student_loss)).requires_grad_(True)
        else:
            loss = student_loss.requires_grad_(True)
...
def distill_whisper(cfg: DictConfig):
    ...
    # Enable training for all student model parameters
    student_model.train()
    ...
    logger.info(f"Trainable parameters:  {total_params:,} (100.00%)")  # All parameters should be trainable
    logger.info(f"Frozen parameters:     0 (0.00%)")  # No parameters should be frozen
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        learning_rate_schedule="linear",  # Add learning rate schedule
